...
from transformers import AutoTokenizer, GPT2LMHeadModel, AutoConfig, Trainer, TrainingArguments, GPTNeoXForCausalLM, TrainerCallback
...
from training.utility import FineTuningTestPersonIndexInfoDict
from training.callback import FirstTokenAccuracyCallback, QAExactMatchAccuracyCallback
from training.callback import PreTrainingShuffleQaCallBack, MixedDatasetShuffleCallback
from training.callback import FirstTokenAccuracyCalculationStrategy
from data_module import AttentionMaskType, MixedDataset
from data_module import construct_pre_training_first_token_accuracy_data_module, construct_pre_training_data_module
from data_module import construct_qa_first_token_accuracy_data_module, construct_qa_pre_training_data_module
from data_module import construct_mix_data_pre_training_data_module
...
def train(args):
    ...
    task_config: Dict = pre_training_config['pretrain']  # config that is different for each task
    ...
    training_args = TrainingArguments(
        ...
        save_strategy='no',  # set by myself 🟡
        # save_steps=None,  # set by myself 🟡
        bf16=True,  # set by myself 🟡
        logging_steps=5,  # set by myself 🟡
    ...
    info_dict = task_config['person_index_info_list'][0]
    BIO_counts = info_dict['end'] - info_dict['start']
    qar = task_config["QAr"]
    wandb_name = f"QAr_{qar}_maxstep_{max_steps}_bio_{BIO_counts}"
    ...
    additional_training_args = AdditionalTrainingArguments(
        ...
        first_token_accuracy_calculation_strategy=FirstTokenAccuracyCalculationStrategy[
            task_config['first_token_accuracy_calculation_strategy']],
        first_token_accuracy_calculation_interval=task_config['first_token_accuracy_calculation_interval'],
        pre_training_person_index_info_list=task_config['person_index_info_list'],
        pre_training_qa_info_list=task_config['qa_index_info_list'],
        test_person_index_info_list=task_config['test_qa_index_info_list'],
    )
    ...
    data_args = DataArguments(
        ...
        all_qa_data_path=shared_config['qa_data_path']
    )
    # process dataset
    # bio_dataset = construct_pre_training_data_module(tokenizer, data_args, additional_training_args)
    # qa_dataset = construct_qa_pre_training_data_module(tokenizer, data_args, additional_training_args)
    mix_dataset  = construct_mix_data_pre_training_data_module(tokenizer, data_args, additional_training_args, qar)
    mix_dataset.get_input_ids_list_info()
    #  bio first token accuracy
    # bio_first_token_accuracy_dataset = construct_pre_training_first_token_accuracy_data_module(
    #     tokenizer, data_args, additional_training_args)
    # bio_first_token_accuracy_callbacks = [FirstTokenAccuracyCallback(bio_first_token_accuracy_dataset,
    ...
    # qa_train first token accuracy
    qa_first_token_accuracy_dataset = construct_qa_first_token_accuracy_data_module(
        tokenizer, data_args, additional_training_args, tokenizer.model_max_length)
    qa_first_token_accuracy_callbacks = [FirstTokenAccuracyCallback(qa_first_token_accuracy_dataset,
                                        additional_training_args.first_token_accuracy_calculation_strategy,
                                        additional_training_args.first_token_accuracy_calculation_interval)]

    # qa_test exact match accuracy
    qa_exact_match_dataset = construct_qa_exact_match_data_module(data_args, additional_training_args)
    qa_exact_match_callbacks = [QAExactMatchAccuracyCallback(qa_exact_match_dataset, tokenizer)]

    callbacks: list[TrainerCallback | type[TrainerCallback]] =  qa_first_token_accuracy_callbacks
    callbacks += [MixedDatasetShuffleCallback]
    trainer = Trainer(
        model=model,
        tokenizer=tokenizer,
        args=training_args,
    ...
    info_dict = task_config['person_index_info_list'][0]
    BIO_counts = info_dict['end'] - info_dict['start']
    model_name = f"QAr_{qar}_maxstep_{max_steps}_bio_{BIO_counts}_model"
    train_and_save_model(trainer, training_args, model_name, remove_all_checkpoint=True, save_final_model=True)
    # endregion
