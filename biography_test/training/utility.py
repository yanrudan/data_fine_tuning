...
PreTrainingQAIndexInfoList = List[PersonIndexInterval]
...
FineTuningTestPersonIndexInfoList = List[PersonIndexInterval]
...
class AdditionalTrainingArguments:
    ...
    pre_training_qa_info_list: PreTrainingPersonIndexInfoList = field(
        default=None,
        metadata={"help": "List of qa ids for pre-training."}
    )
    ...
    fine_tuning_validation_person_index_info_list: FineTuningTestPersonIndexInfoList = field(
        default=None,
        metadata={"help": "List of biography ids for fine-tuning validation."
    ...
    fine_tuning_test_person_index_info_list: FineTuningTestPersonIndexInfoList = field(
        default=None,
        metadata={"help": "List of biography ids for fine-tuning testing. "
                          "The testing will be carried out ONLY at the end of the fine-tuning process."}
    )
    test_person_index_info_list: FineTuningTestPersonIndexInfoList = field(
        default=None,
        metadata={"help": "List of biography ids for testing. "
                          "The testing will be carried out ONLY at the end of the pre-training process."}
    )
...
def train_and_save_model(trainer: Trainer, training_args: TrainingArguments, model_name: str, remove_all_checkpoint: bool, save_final_model: bool):
    ...
    trainer.save_state()
    if save_final_model:
        if not os.path.exists(os.path.join(training_args.output_dir, model_name)):
            os.makedirs(os.path.join(training_args.output_dir, model_name))
        safe_save_model_for_hf_trainer(trainer=trainer, output_dir=os.path.join(training_args.output_dir, model_name))
